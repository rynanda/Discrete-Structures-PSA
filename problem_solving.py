...
def censor(s):
   ...
   r = re.subn(r"\ban\b", "##", r, flags = re.IGNORECASE)[0]
   r = re.subn(r"\bthe\b", "###", r, flags = re.IGNORECASE)[0]
   if r == s:
      return s
   else:
      return r + " <n10852565>"
...
def fertiliser(an, ap, bn, bp, n, p):
   amounts = np.array([[an, bn], [ap, bp]])
   try:
      invAmounts = np.linalg.inv(amounts)
   except:
      return None
   required = np.array([[n], [p]])
   amountsRequired = invAmounts @ required
   a = amountsRequired[0, 0]
   b = amountsRequired[1, 0]
   if a >= 0 and b >= 0:
      return a, b
   else:
      return None
